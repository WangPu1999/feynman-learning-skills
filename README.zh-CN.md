...
真正有效的学习，不是你看到解释时觉得“我懂了”，而是你的大脑在不看原文的情况下，能够重新搭建这个概念。这个过程需要提取、简化、犯错、修补、迁移、边界意识和破坏。当你用简单语言解释一个概念时，大脑被迫判断什么最重要，暴露推理链条在哪里断裂，并把知识连接到你真的能使用的例子上。费曼学习法之所以有效，是因为它把学习从“认得出来”变成“重新生成”。科研和数学学习还要再往前走一步：当你理解标准版本后，继续追问它在哪里失效、极限情况会怎样、它是谁的特例、更一般的母体是什么。读论文和流程时，还要问 baseline 是什么，作者为什么这样设计，如果是我会怎么改，哪里会坏，需要什么实验验证。真正的研究问题，往往就是在这种边界和结构扰动中出现的。
...
- 做边界与母体检查：参数趋近极限时会怎样，假设变强/变弱时会怎样，它是谁的特例，兄弟概念是什么。
- 用破坏式思考检查：如果前提、组件、机制、边界或领域发生变化，结论会怎样改变。
- 读论文或流程时，识别 baseline、设计理由、可修改环节、失败风险和验证实验。
- 引导学习者关注自己的好奇心、兴趣和熟悉领域，用熟悉场景建立连接。

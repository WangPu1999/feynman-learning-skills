...
真正有效的学习，不是你看到解释时觉得“我懂了”，而是你的大脑在不看原文的情况下，能够重新搭建这个概念。这个过程需要提取、简化、犯错、修补、迁移和破坏。当你用简单语言解释一个概念时，大脑被迫判断什么最重要，暴露推理链条在哪里断裂，并把知识连接到你真的能使用的例子上。费曼学习法之所以有效，是因为它把学习从“认得出来”变成“重新生成”。科研学习还要再往前走一步：当你理解标准版本后，主动改变一个前提、去掉一个组件、反转一个机制，或把概念搬到另一个领域。读论文和流程时，这意味着追问：baseline 是什么，作者为什么这样设计，如果是我会怎么改，哪里会坏，需要什么实验验证。真正的研究问题，往往就是在这种结构扰动中出现的。
